package com.employee.videoeditor.adapters.photoeditor;
...
import com.employee.videoeditor.R;
...
//                frmBorder.setTag(!isBackgroundVisible);
    ...
    public void addText(@Nullable Typeface textTypeface, String text, final int colorCodeTextView, int tag) {
        ...
        addText(text, styleBuilder, tag, false, 0, false, 0,0);
    }
...
//    }
    ...
    public void addText(String text, @Nullable TextStyleBuilder styleBuilder, int position, boolean status, int textSize, boolean bgStatus, int bgColor, int textAllign) {
        brushDrawingView.setBrushDrawingMode(false);
        final View textRootView = getLayout(ViewType.TEXT);
        final TextView textInputTv = textRootView.findViewById(R.id.tvPhotoEditorText);
        textInputTv.setTag(position);
...
//        textInputTv.setGravity(Gravity.CENTER);
        ...
        if (bgStatus) {
            textInputTv.setBackgroundColor(bgColor);
        } else {
            textInputTv.setBackgroundColor(Color.TRANSPARENT);
        }

        if (textAllign == 0) {
            textInputTv.setGravity(Gravity.CENTER);
        } else if (textAllign == 1) {
            textInputTv.setGravity(Gravity.RIGHT);
        } else if (textAllign == 2) {
            textInputTv.setGravity(Gravity.LEFT);
        }
